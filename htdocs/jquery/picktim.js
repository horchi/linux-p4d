...
(function ($) {
   ...
   function Plugin(element, options) {
      ...
      this.incrementInterval = 400;
      this.incrementUpCount = 0;
      this.incrementValue = 1;      // for hours
      this.incrementValueMin = 10;  // for minutes
      ...
      if (this.settings.width != null)
         this.tPopup.get(0).style.setProperty('--picktim-container-width', this.settings.width);
      else if (this.settings.mode === this.MODES.h24)
      ...
      this.tPopup.get(0).style.setProperty('--picktim-container-position', this.settings.position);
      this.setOrientation();
      this.tPopup.hide();
   ...
   Plugin.prototype.checknums = function(e) {
      ...
      /*
        if (m.val() % this.incrementValueMin) {
        var v = parseInt(m.val()/this.incrementValueMin, 10) * this.incrementValueMin;
        m.val(('0' + v).slice(-2));
        m.data['val'] = v;
        }*/
   }
   ...
   Plugin.prototype.checkTime = function(e)
   {
      ...
      if (this.tMins.val() % this.incrementValueMin) {
         var v = parseInt(this.tMins.val()/this.incrementValueMin, 10) * this.incrementValueMin;
         this.tMins.val(('0' + v).slice(-2));
         this.tMins.data['val'] = v;
         this.updateInput();
      }
   }
   ...
   Plugin.prototype.iDL = function(v, inc, i, lim, step = 1)
   {
      if (inc) {       // increment
   ...
   {
      this.tMins.val(this.iDL(this.tMins.val(), true, e, 60, this.incrementValueMin));
   ...
   Plugin.prototype.decrementMinutes = function(e){
      this.tMins.val(this.iDL(this.tMins.val(), false, e, 60, this.incrementValueMin));
      this.tMins.trigger('change');
   };
