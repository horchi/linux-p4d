...
int P4Request::getMenuItem(MenuItem* m, int first)
{
   ...
   if ((status = readByte(m->b1)) != success)
   {
      tell(eloAlways, "Missing at least %d bytes at reading b1 (byte)", size);
      show("<- ");
      return status;
   }

   if ((status = readByte(m->b2)) != success)
   {
      tell(eloAlways, "Missing at least %d bytes at reading b1 (byte)", size);
      show("<- ");
      return status;
